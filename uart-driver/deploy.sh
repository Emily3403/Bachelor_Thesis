...
if [ "$REALTIME" == "true" ];
then
    ssh -t $RASPI_CUSTOM_KERNEL_HOST "chrt 99 ./driver-binaries/$BINARY_NAME -b '$BAUDRATE' --savedir '$SAVEDIR'"
else
    ssh -t $RASPI_CUSTOM_KERNEL_HOST "./driver-binaries/$BINARY_NAME -b '$BAUDRATE' --savedir '$SAVEDIR'"
fi
